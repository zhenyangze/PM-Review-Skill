---
name: pm-review
description: 产品经理页面评审工具，用于评估网页或设计截图的产品质量。触发条件：用户要求评审页面、分析截图、评估设计、PM评审、产品评审、界面分析。支持两种输入方式：(1) 网址评审 - 通过 URL 评审在线页面 (2) 截图评审 - 通过图片文件评审设计稿。输出七维度专业评审报告（战略目标/用户体验/功能内容/数据度量/技术性能/市场竞品/迭代运营）。
---
...
读取评分标准和检查清单：
- [references/dimensions.md](references/dimensions.md) - 七维度评分标准
